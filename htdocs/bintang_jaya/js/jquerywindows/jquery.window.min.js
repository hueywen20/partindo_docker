...
$.Window=(function(){var VERSION="5.01a";var ICON_WH=16;var ICON_MARGIN=4;var ICON_OFFSET=ICON_WH+ICON_MARGIN;var OPACITY_MINIMIZED=0.7;var MINIMIZED_NARROW=24;var MINIMIZED_LONG=120;var RESIZE_EVENT_DELAY=200;var ua=navigator.userAgent.toLowerCase();var windowIndex=0;var lastSelectedWindow=null;var windowStorage=[];var initialized=false;var resizeTimer=null;var parentCallers=[];var minWinData={long:MINIMIZED_LONG,storage:[]};var setting={dock:'left',animationSpeed:400,minWinNarrow:MINIMIZED_NARROW,minWinLong:MINIMIZED_LONG,handleScrollbar:true,showLog:false};function selectWindow(parent,wnd){if(parent==null){if(lastSelectedWindow!=null&&lastSelectedWindow!=wnd){lastSelectedWindow.unselect();wnd.select();}else if(lastSelectedWindow==null){wnd.select();}
lastSelectedWindow=wnd;}else if(parent!=null){if(parent.get(0)._lastSelectedWindow!=null&&parent.get(0)._lastSelectedWindow!=wnd){parent.get(0)._lastSelectedWindow.unselect();wnd.select();}else if(parent.get(0)._lastSelectedWindow==null){wnd.select();}
...
function pushMinWindow(parent,wnd){if(parent!=null){parent.get(0)._minWinData.storage.push(wnd);}else{minWinData.storage.push(wnd);}}
function popMinWindow(parent,wnd){var doAdjust=false;var storage=(parent!=null)?parent.get(0)._minWinData.storage:minWinData.storage;for(var i=0;i<storage.length;i++){var w=storage[i];if(w==wnd){storage.splice(i--,1);doAdjust=true;continue;}
if(doAdjust){w._decreaseMiniIndex();}}}
function getMinWindowLength(parent){var storage=(parent!=null)?parent.get(0)._minWinData.storage:minWinData.storage;return storage.length;}
function checkMinWindowSize(parent,bPush){var bAdjust=false;var rect=null;var mwdata=minWinData;if(parent!=null){rect={width:parent.innerWidth(),height:parent.innerHeight()};mwdata=parent.get(0)._minWinData;}else{rect=getBrowserScreenWH();}
...
function adjustAllMinWindows(parent){var storage=(parent!=null)?parent.get(0)._minWinData.storage:minWinData.storage;for(var i=0;i<storage.length;i++){storage[i]._adjustMinimizedPos(false);}}
...
function getCssStyleByDock(parent,miniIndex){var targetCss={};var screenWH=getBrowserScreenWH();var cpos=null;var bTop=0;var bLeft=0;var narrow=setting.minWinNarrow;var long=minWinData.long;if(parent!=null){cpos=parent.offset();bTop=parseInt(parent.css('borderTopWidth'));bLeft=parseInt(parent.css('borderLeftWidth'));long=parent.get(0)._minWinData.long;}
...
function constructor(caller,options){var _this=null;var windowId="window_"+(windowIndex++);var minimized=false;var maximized=false;var selected=false;var redirectCheck=false;var pos=new Object();var wh=new Object();var orgPos=new Object();var orgWh=new Object();var targetCssStyle={};var headerFuncPanel=null;var funcBarWidth=0;var miniStackIndex=-1;var animating=false;var container=null;var header=null;var frame=null;var footer=null;var options=$.extend({title:"",url:"",content:"",footerContent:"",containerClass:"",headerClass:"",frameClass:"",footerClass:"",selectedHeaderClass:"",x:-1,y:-1,z:2000,width:400,height:300,minWidth:200,minHeight:150,maxWidth:800,maxHeight:600,showFooter:true,showRoundCorner:false,closable:true,minimizable:true,maximizable:true,bookmarkable:true,draggable:true,resizable:true,scrollable:true,checkBoundary:false,custBtns:null,onOpen:null,onShow:null,onClose:null,onSelect:null,onUnselect:null,onDrag:null,afterDrag:null,onResize:null,afterResize:null,onMinimize:null,afterMinimize:null,onMaximize:null,afterMaximize:null,onCascade:null,afterCascade:null,onIframeStart:null,onIframeEnd:null,iframeRedirectCheckMsg:null,createRandomOffset:{x:0,y:0}},options);function initialize(instance){_this=instance;var realCaller=caller!=null?caller:$("body");var cornerClass=options.showRoundCorner?"ui-corner-all ":"";realCaller.append("<div id='"+windowId+"' class='window_panel "+cornerClass+options.containerClass+"'></div>");container=realCaller.children("div#"+windowId);if($.isFunction(options.onOpen)){options.onOpen(_this);}
...
if(options.x>=0||options.y>=0){var scrollPos=getBrowserScrollXY();if(options.x>=0){var pLeft=0;if(caller!=null){pLeft=options.x+caller.offset().left;}else{pLeft=options.x+scrollPos.left;}
container.css("left",pLeft);}else{alignHorizontalCenter();}
if(options.y>=0){var pTop=0;if(caller!=null){pTop=options.y+caller.offset().top;}else{pTop=options.y+scrollPos.top;}
...
hideOverlay();showContent();if(options.afterDrag){options.afterDrag(_this);}}});if(options.checkBoundary){container.draggable('option','containment','parent');}}
...
var textBlock=header.children("div.window_title_text");var text=options.title;var buf="";for(var i=0;i<text.length;i++){var c=text.charAt(i);if(c=="-"||c=="_"){c="|";}
...
function alignHorizontalCenter(){var pLeft=0;if(caller!=null){pLeft=caller.offset().left+(caller.width()-container.width())/2;}else{var scrollPos=getBrowserScrollXY();var screenWH=getBrowserScreenWH();pLeft=scrollPos.left+(screenWH.width-container.width())/2;}
container.css({left:pLeft});}
function alignVerticalCenter(){var pTop=0;if(caller!=null){pTop=caller.offset().top+(caller.height()-container.height())/2;}else{var scrollPos=getBrowserScrollXY();var screenWH=getBrowserScreenWH();pTop=scrollPos.top+(screenWH.height-container.height())/2;}
...
headerFuncPanel.hide();hideContent();checkMinWindowSize(caller,true);_adjustMinimizedPos(false,function(){container.css('z-index',options.z);header.children("div.window_title_text").width("100%");header.attr("title",options.title);header.removeClass('window_header_normal');header.removeClass('window_header_maximize');header.addClass('window_header_minimize');if(setting.dock=='left'||setting.dock=='right'){header.addClass('window_header_minimize_vertical');}
...
function restore(){var rpos=null;var rwh=null;var zIndex=options.z+2;if(minimized){rpos=orgPos;rwh=orgWh;transferToAbsolute();header.removeClass('window_header_minimize');if(setting.dock=='left'||setting.dock=='right'){header.removeClass('window_header_minimize_vertical');}
...
restoreTitleText();header.removeAttr("title");header.removeClass('window_header_minimize');if(setting.dock=='left'||setting.dock=='right'){header.removeClass('window_header_minimize_vertical');}
...
return instance;},getVersion:function(){return VERSION;},prepare:function(custSetting){$.extend(setting,custSetting);minWinData.long=setting.minWinLong;},closeAll:function(quiet){var count=windowStorage.length;for(var i=0;i<count;i++){var wnd=windowStorage[0];wnd.close(quiet);}
windowStorage=[];minWinData.storage=[];minWinData.long=setting.minWinLong;for(var i=0;i<parentCallers.length;i++){var mwdata=parentCallers[i].get(0)._minWinData;mwdata.storage=[];mwdata.long=setting.minWinLong;}},hideAll:function(){for(var i=0,len=windowStorage.length;i<len;i++){windowStorage[i].getContainer().hide();}},showAll:function(){for(var i=0,len=windowStorage.length;i<len;i++){windowStorage[i].getContainer().show();}},getAll:function(){return windowStorage;},getWindow:getWindow,getSelectedWindow:function(){for(var i=0,len=windowStorage.length;i<len;i++){var wnd=windowStorage[i];if(wnd.isSelected()){return wnd;}}}}})();$.window.getVersion=$.Window.getVersion;$.window.prepare=$.Window.prepare;$.window.closeAll=$.Window.closeAll;$.window.hideAll=$.Window.hideAll;$.window.showAll=$.Window.showAll;$.window.getAll=$.Window.getAll;$.window.getWindow=$.Window.getWindow;$.window.getSelectedWindow=$.Window.getSelectedWindow;