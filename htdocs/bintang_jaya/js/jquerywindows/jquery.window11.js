/**
 * jQuery Window Plugin - To Popup A 	 Window-like Dialog
 * http://fstoke.me/jquery/window/
 * Copyright(c) 2010 David Hung
 * Dual licensed under the MIT and GPL licenses
 * Version: 5.01a
 * Last Revision: 2010-11-19
 *
 ...
 * Customized Button JSON Array Sample:*/
	var myButtons = [
		// reload button
		{
		id: "btn_reload",           // required, it must be unique in this array data
		title: "reload",   // optional, it will popup a tooltip by browser while mouse cursor over it
		...
		image: "js/jquerywindows/img/reload.gif",    // optional, the image url of button icon(16x16 pixels)
		callback:                     // required, the callback function while click it
			function(btn, wnd) {
				wnd.getFrame().attr("src",wnd.getUrl());
				//wnd.setContent(wnd.getContent());
			}
		},
	];
...
$.Window = (function()  {
	...
	// static constants
	var VERSION = "5.01a";          // the version of current plugin
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
			...
			x: 5,                        // [number:-1] the x-axis value on screen(or caller element), if -1 means put on screen(or caller element) center
			y: 40,                        // [number:-1] the y-axis value on screen(or caller element), if -1 means put on screen(or caller element) center
			...
			custBtns: myButtons,               // [json array:null] to describe the customized button display & callback function
			...
			createRandomOffset: {x:100, y:100} // [json object:{x:0, y:0}] random the new created window position, it only works when options x,y value both are -1
		}, options);
		...
		function initialize(instance) {
			...
			if( options.bookmarkable && options.url != null && $.trim(options.url) != "" ) {
				headerFuncPanel.append( "<div title='refresh' class='bookmarkImg window_icon_button no-draggable'></div>" );
				headerFuncPanel.children(".bookmarkImg").click(function() {
					//doBookmark(options.title, options.url);
					//alert(_this.getContent());
		...
		function minimize() {
			...
			_adjustMinimizedPos(false, function() {
				container.css('z-index', options.z+4);
				//container.css('z-index', '8888');
				...
				header.css('z-index', options.z+4);
				if( setting.dock == 'left' || setting.dock == 'right' ) {
		...
		function showthenavontop(){
			for( var i=0, len=windowStorage.length; i<len; i++ ) {
				if (windowStorage[i].isMinimized()){
					windowStorage[i].getContainer().css("z-index","8888");
				}
			}
		}
	...
	return { // static public methods
		...
		minAll: function() { // minimize all created windows
			for( var i=0, len=windowStorage.length; i<len; i++ ) {
				if (!windowStorage[i].isMinimized())
					windowStorage[i].minimize();
			}
		},
		searchWindow: function(url) {
			var foundit = false;
			for( var i=0, len=windowStorage.length; i<len; i++ ) {
				if (windowStorage[i].getUrl() == url){
					if (!windowStorage[i].isMinimized())
						selectWindow(null,windowStorage[i]);
					else
						windowStorage[i].restore();
					foundit = true;
					break;
				}
			}
