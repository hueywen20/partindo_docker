...
function dhtmlXCombo(parent,name,width,optionType,tabIndex){
 ...
 dhtmlXCombo.prototype.setSize = function(new_size){
	...
	this.DOMlist.style.width="auto";
	this.DOMlist.style.paddingRight="15px";
	if (this.DOMlistF)
		this.DOMlistF.style.width="auto";	
};
...
dhtmlXCombo.prototype.destructor = function()
...
};dhtmlXCombo.prototype._createSelf = function(selParent, name, width, tab)
 {
	...
	this.DOMelem_input.onfocus = this._onKey;
...
dhtmlXCombo.prototype._listOver = function(e)
 {
	...
	var that = this.combo;
	if ( node.parentNode == that.DOMlist.childNodes[0] ){
		if(that._selOption)
			that._selOption.deselect();
		if(that._tempSel)
			that._tempSel.deselect();
		var i=0;
		for (i;i<that.DOMlist.childNodes[0].childNodes.length;i++){
			if (that.DOMlist.childNodes[0].childNodes[i]==node)
				break;
		};
	...
	var ckin = checkinnode(node.parentNode,that.DOMlist.childNodes[0].childNodes);
	if ( ckin != -1 ){
		if(that._selOption)
			that._selOption.deselect();
		if(that._tempSel)
			that._tempSel.deselect();
		var z=that.optionsArr[ckin];
		that._tempSel=z;
		that._tempSel.select();
		if ((that._autoxml)&&((i+1)==that._lastLength)){
			that._fetchOptions(i+1,that._lasttext||"");
...
};dhtmlXCombo.prototype.getPosition = function(oNode,pNode){if (_isChrome){if(!pNode)var pNode = document.body
 ...
 dhtmlXCombo.prototype._onKeyF = function(e){
	...
	var ev=e||event;
	ev.cancelBubble=true;
	if (ev.keyCode=="13" || ev.keyCode=="9" || ev.keyCode=="39" ){
	...
	if (ev.keyCode=="13" || ev.keyCode=="27" || ev.keyCode=="39" ){
		this.blur();
...
dhtmlXCombo.prototype._onKey = function(e){
	...
	if (ev==27)
		return;
	if ((that.DOMlist.style.display!="block")&&(ev!="13")&&(ev!="9")&&(ev!="39")&&((!that._filter)||(that._filterAny)))
...
dhtmlXCombo.prototype._onKeyB = function(ev)
 ...
 if (this._filter)return this.filterSelf((ev==8)||(ev==46));for(var i=0;i<this.optionsArr.length;i++)if (this.optionsArr[i].data()[1]==this.DOMelem_input.value){this.selectOption(i,false,false);return false;};this.unSelectOption();};return true;};
 ...
 dhtmlXCombo.prototype.redrawOptions = function(){
	...
	AF.style.width="108%";
	//AF.style.border="1px solid #000";
	this.DOMlist.appendChild(AF);
	for(var i=0;i<this.optionsArr.length;i++)
		AF.appendChild(this.optionsArr[i].render());
	//this.DOMlist.style.width = parseInt(parseInt(AF.style.offsetWidth)+parseInt(100))+"px";
	this.DOMlist.style.paddingRight = "50px";
};
...
dhtmlXCombo.prototype._fillFromXML = function(obj,b,c,d,xml){
	...
	if ((!top[0])||(!top[0].getAttribute("add"))){
		...
		obj._lastLength+=options.length||Infinity;add = true;
	};
	...
	obj.unSelectOption();
	obj.callEvent("onXLE",[]);
};dhtmlXCombo.prototype.unSelectOption = function(){if (this._selOption)this._selOption.deselect();if(this._tempSel)this._tempSel.deselect();this._tempSel=this._selOption=null;};dhtmlXCombo.prototype.confirmValue = function(){this._confirmSelection();};
 dhtmlXCombo.prototype._confirmSelection = function(data,status){
	if(arguments.length==0){
		//var z=this.getOptionByLabel(this.DOMelem_input.value);
		//alert(z);
		var z = this.getSelectedValue();
		//data = z?z.value:this.DOMelem_input.value;
		data = z?z:this.DOMelem_input.value;
		status = (z==null);
	...
	var AR = this.getSelectedText().split("|^|");
	var y = AR[0];
	//alert(this._selOption.data());
	if (y && AR.length == 1){
		this.DOMelem_input.value=y;
		this.DOMelem_hidden_input.value=this.getSelectedValue();
		//this.setComboText(y);
	}
	else{
		this.DOMelem_input.value=y;
...
dhtmlXCombo.prototype.openSelect = function(){
	...
	//alert(this);
	//dhtmlXCombo('','combo',0,e,tabIndex);
	//this.DOMList._createSelf('','combo',0,'');
	//var options=xml.doXPath("//option");
	//alert(this.DOMParent.innerHTML);
	/*var add = false;
	obj.render(false);
	if ((!top[0])||(!top[0].getAttribute("add"))){
		obj.clearAll();
	...
	/*if (this._filter)
		this.filterSelf();*/
	//alert(this.DOMlist.style.display);
};
...
dhtmlXCombo.prototype._fetchOptions=function(ind,text){
	...
	if (this._load)
		...
		if (!this.callEvent("onDynXLS",[text,ind])) 
			return;
		this.loadXML(url);
	}
};
...
dhtmlXCombo_defaultOption.prototype.render = function(){
	if (!this.content){
		//if (this.text.indexOf("|^|") != -1){
			...
			this.content.style.cssText='overflow:hidden;'+this.css;
			if (_isOpera || _isKHTML )
			...
			for (var h = 0; h < textarr.length; h++){
				var VB = document.createElement("TD");
				if (h == 4){
					VB.style.textAlign = "right";
				}
				else{
					VB.style.textAlign = "left";
				}
				VB.style.cursor = "default";
				if (h > 0){
					VB.style.borderLeft = "1px solid #333";
				}
				VB.innerHTML = textarr[h];
				this.content.appendChild(VB);
				//texthtml += '<td align="left">'++'</td>';
			}
			//this.content.innerHTML=texthtml;
		/*}
		else{
...
dhtmlXCombo.prototype.dhx_Event=function()
{
	...
	this.callEvent=function(name,arg0){
		if (this["ev_"+name])
			return this["ev_"+name].apply(this,arg0);return true;
	};
...
(function(){dhtmlx.extend_api("dhtmlXCombo",{_init:function(obj){
	if (obj.image_path)
