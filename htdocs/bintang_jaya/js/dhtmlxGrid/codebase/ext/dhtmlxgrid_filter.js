...
dhtmlXGridObject.prototype._filterA=function(column,value){
	...
	var FD;
	var PSVB;
	var i = this.rowsBuffer.length-1;
	while (i >= 0){
		PSVB = this.rowsBuffer[i].idd+"";
		VB = PSVB.indexOf("_");
		if (VB != -1){
			PL = PSVB.substr(0,VB);
			LT = PSVB.substr(VB+1);
			IX = parseInt(parseInt(LT)+parseInt(1));
			FD = false;
			for (var j=i; j > (i-IX); j--){
				FG = this._get_cell_value(this.rowsBuffer[j],column).toString().toLowerCase().indexOf(value);
				if (FG == 0){
					FD = true;
					break;
				}
			...
			if (!FD){
				j++;
				this.rowsBuffer.splice(j,IX);
			}
			i -= IX;
		}
		else{
			FG = this._get_cell_value(this.rowsBuffer[i],column).toString().toLowerCase().indexOf(value);
			if (d?(FG!=0):(!value(this._get_cell_value(this.rowsBuffer[i],column),this.rowsBuffer[i].idd))){
	...
	/*for (var i=this.rowsBuffer.length-1;i>=0;i--){
		FG = this._get_cell_value(this.rowsBuffer[i],column).toString().toLowerCase().indexOf(value);
		VB = this.rowsBuffer[i].idd.indexOf("_");
		if (VB != -1){
			PL = this.rowsBuffer[i].idd.substr(0,VB);
			EX = s_in_array(PL,rowids);
		}
		else{
			PL = this.rowsBuffer[i].idd;
			EX = s_in_array(this.rowsBuffer[i].idd,rowids);
		}
...
dhtmlXGridObject.prototype.findCell=function(value, c_ind, count){var res = new Array();value=value.toString().toLowerCase();if (typeof count != "number")count = count?1:0;if (!this.rowsBuffer.length)return res;for (var i = (c_ind||0);i < this._cCount;i++){if (this._h2)this._h2.forEachChild(0,function(el){if (count && res.length==count)return res;if (this._get_cell_value(el.buff,i).toString().toLowerCase().indexOf(value) != -1){res.push([el.id,i]);}},this)
 ...
 id=document.getElementById(id);if(!id)return;var self=this;
 if (id.tagName=='INPUT'){
