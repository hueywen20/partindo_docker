...
function editingaout(stage,rId,cInd){
	if (stage == 0){
		/* if (cInd == 1){
		...
		if (cInd == 0){
			...
			if (cInd == 5){
				...
				if (parseFloat(qtytemp) > parseFloat(parseFloat(arrmaxr[rId]) + parseFloat(tempqty))){
					alert("Jumlah barang yang disesuaikan tidak bisa lebih besar dari sisa stok, sisa stok : "+formatnumber(arrmaxr[rId]));
					return false;
				}
