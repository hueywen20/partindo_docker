...
<script type="text/javascript">
		...
		if (!$(".formID").validationEngine("validate")){
			...
			if ($("#customercode").val() == ""){
				...
				if (mygrid.getRowsNum() == 0){
					alert("Tidak ada data penagihan");
				}
				else{
	...
	function blurdate(){
		...
		if (getpaymentdate.length  != 10){
			alert("Tanggal Penagihan tidak lengkap. Tanggal Penagihan akan dibuat hari ini.");
	...
	function checkmaximumdate(){
		...
		if (getpaymentdate != ""){
			...
			if (bdate > dtnow){
				alert("Tanggal Penagihan melewati hari ini");
				$("#paymentdate").val(setLengthDate(datenow.getDate(), 2) + dateSplitter + setLengthDate(datenow.getMonth() + 1, 2) + dateSplitter + setLengthDate(datenow.getFullYear(), 4));
			}
	...
	alert ("Nilai Pembayaran Yang Anda Masukkan Kurang Dari Nilai Piutang");
	}
	else{
	if (confirm("Data sudah benar?")){
	//objfrm.submit();
...
<body>
<if criteria="!empty($_REQUEST[no]) && $useraccess[add_payment]">
<div align="right">
<input type="button" value="Tambah Baru" class="button" onclick="window.open(\'payment.php\',\'_self\')"></div></endif>
<form class="formID" id="formID" name="payment" action="payment.php" method="post" onsubmit="return checkval(this)">
<fieldset>
	<legend>Informasi Penagihan</legend>
	<table border="0" cellpadding="2" cellspacing="2">
	<if criteria="empty($headerpayment[hpid])">
	<tr>
		<td align="left">Tanggal Penagihan</td>
		<td align="left">
	...
	<else>
	<tr>
		<td align="left">Nomor Penagihan</td>
		<td align="left">
	...
	<if criteria="!empty($suppliername)">
	<tr>
		...
		<tr>
			...
			<input type="hidden" name="supplieraddrid" id="supplieraddrid" value="$headerpaydebt[supplieraddrid]">
	...
	<tr>
		...
		<img onclick="searchsalelist()" src="img/icon_view.png" border="0" style="cursor: pointer">
...
<fieldset>
	<legend>Daftar Faktur Penjualan</legend>
	<div align="left">
		<table border="0" cellpadding="3" cellspacing="3">
		<tr>
			<td align="left" class="tdhover" onclick="deletedetailpayment()"><img src="img/delete.png" border="0">&nbsp;&nbsp;<b>Hapus</b></td>
		</tr>
		</table>
	</div>
		...
		mygrid.attachEvent("onAfterRowDeleted", function(id,pid){
			/* if (id.indexOf("sale_") != -1){
				...
				while (true){
					...
					if (idnow.indexOf("sale_") != -1 || idnow.indexOf("buy_") != -1){
						break;
					}
					mygrid.deleteRow(idnow);
				}
			}
			else if (id.indexOf("buy_") != -1){
				var idnow;
				while (true){
					idnow = mygrid.getRowId(indexdels);
					if (typeof(idnow) == "undefined"){
...
<fieldset>
	...
	<div align="left">
		<table border="0" cellpadding="3" cellspacing="3">
		<tr>
			<td align="left" class="tdhover" onclick="addrepayment()"><img src="img/add.png" border="0">&nbsp;&nbsp;<b>Tambah</b></td>
			<td align="left" class="tdhover" onclick="deleterepayment()"><img src="img/delete.png" border="0">&nbsp;&nbsp;<b>Hapus</b></td>
		</tr>
		</table>
	</div>
	...
	<script>
		...
		mygridrp.loadXML("payment.php?getlist=xml&list=detailrepay&no=$_REQUEST[no]");
	</script>
</fieldset><br>
<table border="0" width="100%" cellpadding="0" cellspacing="0">
<tr>
	...
	<tr >
		...
		<td align="right" width="30%">
		<input type="text" name="flat" autocomplete="off" id="flat" style="text-align: right" onkeypress="return checknumber(event)" onkeyup="insertingfs(this);fillremaining()" value="$hflat">
		</td>
	</tr>

	<tr >

		<td align="right" width="70%"><b>Kelebihan Bayar Piutang Bulan Ini (Rp.) :</b></td>
		<td align="right" width="30%">
		<input type="hidden" name="remainingnow" id="remainingnow" value="$headerpayment[remainingnow]">
...
<div align="center">
	...
	<div align="left" style="padding: 10px 0">Gunakan kode faktur penjualan di tanda terima ?
	<input type="radio" name="usecode" id="usecode" value="yes" checked><label for="usecode">Ya</label>
	<input type="radio" name="usecode" id="nocode" value="no"><label for="nocode">Tidak</label></div>
	<input type="hidden" name="submits" value="Ubah">
	<input type="submit" value="Ubah" class="button">&nbsp;&nbsp;&nbsp;</endif>
	<input type="button" value="Kembali" class="button" onclick="window.open(\'payment.php?screen=list\',\'_self\')">&nbsp;&nbsp;&nbsp;
	<input type="button" value="Print Dengan Header" class="button" onclick="window.open(\'printpayment.php?no=$_REQUEST[no]&op=yes&usecode=\'+$(\'input[name=usecode]:checked\').val()
,\'_blank\')">&nbsp;&nbsp;&nbsp;
	<input type="button" value="Print Tanpa Header" class="button" onclick="window.open(\'printpayment.php?no=$_REQUEST[no]&op=no&usecode=\'+$(\'input[name=usecode]:checked\').val(),\'_blank\')">
	</if>
</div>
</form>
<script>
