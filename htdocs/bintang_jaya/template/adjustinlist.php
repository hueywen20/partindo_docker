...
<script type="text/javascript">
	...
	function convertspan(rowid){
		var ts = 0;
		var ainprice = $("#ainprice_"+rowid+"-"+ts);
		if (ainprice.length > 0){
			var rspan = true;
			while (rspan){
				curvalueainprice = replacestr(replacestr(ainprice.html(),".",""),",",".");
				curvalueainprice = curvalueainprice.replace(".00","");
				ainprice.html(convertcodes(curvalueainprice));
				ts++;
				ainprice = $("#ainprice_"+rowid+"-"+ts);
				if (ainprice.length == 0){
					rspan = false;
				}
			}
	...
	function deconvertspan(rowid){
		...
		if (ainprice.length > 0){
			var rspan = true;
			while (rspan){
				ainprice.html(formatnumber(deconvertcodes(ainprice.html())));
				ts++;
				ainprice = $("#ainprice_"+rowid+"-"+ts);
				if (ainprice.length == 0){
					rspan = false;
...
Pencarian : <select id="searchfield">
	<option value="ainid">Nomor Penyesuaian (+)</option>
	<option value="aindate">Tanggal Penyesuaian (+)</option>
...
<script>
	...
	function ajaxsearch(keywords,fields,page){
		$.get("adjustin.php", {getlist: "xml", list: "general", keyword: keywords, p: page, hdv: heightdiv, field: fields, cwidth: realcolwidth}, function(data){
	...
	function ajaxfromcookie(page){
		...
		var fields = getCookie("fieldain");
		
		var keywords = getCookie("keywordain");
	...
	function searchnow(){
		...
		setCookie("keywordain",keywords);
		setCookie("fieldain",fields);
