...
<script type="text/javascript">
	function checkcodeexist(objfrm){
		if (!$(".formID").validationEngine("validate")){
			...
			$.ajax({
				type: \'POST\',
				url: \'area.php\',
				data: \'check=code&areacode=\'+objfrm.areacode.value+\'&id=\'+ids,
				success: function(data) {
					if (data){
						alert("Kode kota sudah ada dalam sistem. Silahkan masukkan kode kota yang lain.");
						objfrm.areacode.focus();
					}
					else{
						objfrm.submit();
					}
	...
	//window.parent.loadiniframe("supplier.php","area");
	//window.parent.loadiniframe("customer.php","country");
</script>
</head>

<body>
<if criteria="$_GET[getlist] == \'detail\' && ($useraccess[add_city] || ($useraccess[edit_city] && !empty($_REQUEST[id])))">
<if criteria="!empty($errors)">
<div class="error">
<ul>
	<if criteria="$errors == \'samecode\'">
	<li>Kode Area sudah ada dalam sistem. Silahkan masukkan kode area yang lain.</li>
	</endif>
</ul></div><br>
</endif>
<div align="right">
<input type="button" value="Tambah Baru" class="button" onclick="window.open(\'area.php?getlist=detail\',\'_self\')">&nbsp;&nbsp;&nbsp;
<input type="button" value="Kembali" class="button" onclick="window.open(\'area.php\',\'_self\')"></div>
<fieldset>
	<legend><if criteria="!empty($_REQUEST[id])">Ubah Kota<else>Tambah Kota</if></legend>
	<div align="left">
		<form name="frmarea" id="frmarea" class="formID" method="post" action="area.php" onsubmit="return checkcodeexist(this)">
		<table border="0" cellpadding="5" cellspacing="5">
		<tr>
			<td align="left">Kode Kota</td>
			<td align="left">
			<input type="text" name="areacode" id="areacode" value="$detailarea[areacode]" class="validate[required]"></td>
		</tr>
		<tr>
			<td align="left">Nama Kota</td>
			<td align="left">
			<input type="text" name="areaname" id="areaname" value="$detailarea[areaname]" class="validate[required]"></td>
		</tr>
		<tr>
			<td align="left">Status</td>
			<td align="left">
			<select name="areastatus">
				<option value="1"<if criteria="$detailarea[status] == 1"> selected</endif>>Aktif</option>
				<option value="0"<if criteria="$detailarea[status] == 0"> selected</endif>>Non-Aktif</option>
			</select></td>
		</tr>
		<tr>
			<td align="left">
			...
			<if criteria="$useraccess[edit_city]">
...
<else>
<if criteria="$useraccess[add_city]">
...
<input type="button" value="Tambah Baru" class="button" onclick="window.open(\'area.php?getlist=detail\',\'_self\')"></div></endif>
<div id="areabox" style="width: 100%; background-color:white;"></div>
<script>
	var heightauto = (document.documentElement.clientHeight-50)+"px";
	$("#areabox").css("height",heightauto);
	mygrid = new dhtmlXGridObject("areabox");
	mygrid.setImagePath("js/dhtmlxGrid/codebase/imgs/");
	mygrid.setHeader("Kode Kota,Nama Kota,Status,Update Terakhir,Oleh,Action,#cspan");
	...
	mygrid.loadXML("area.php?getlist=xml");
	mygrid.attachEvent("onRowDblClicked", function(rId,cInd){
		var idus = rId.indexOf("_");
		if (idus != -1){
			rId = rId.substr(0,idus);
		}
		window.open("area.php?getlist=detail&id="+rId,"_self");
	});
